...
# add metadata ----
qlist11 = venn2(anno_previos$rn, colnames(combined_seurat_pre))
str(qlist11)
...
if(file.exists(here("results/05_chatti_celltypes.tsv"))==FALSE) {
  ...
  # res_gpt4.5pre_30_ud <- gptcelltype_hk(input = markers_sign,tissuename = "Human Epithelial and Endothelial cells", topgenenumber = 30, model = 'gpt-4.5-preview', include_downreg=TRUE) # 
  
  res_gpt4.5pre_30_ud
  
  chatti = data.table(
    res_gpt4.5pre_30_ud = res_gpt4.5pre_30_ud,
    res_gpt4omini_30_ud=res_gpt4omini_30_ud,
    celltypenum = names(res_gpt4.5pre_30_ud))
  chatti
  ...
  fwrite(chatti, here("results/05_chatti_celltypes.tsv"), sep = "\t")
} else chatti = fread(here("results/05_chatti_celltypes.tsv"))
chatti


combined_seurat$celltype_chatti4_5 = chatti[match_hk(combined_seurat$seurat_clusters, chatti$celltypenum), res_gpt4.5pre_30_ud]
combined_seurat$celltype_chatti4_5 %>% mytable()

seuratanno = combined_seurat@meta.data %>% data.table
seuratanno[, celltype_chatti4_5 := chatti[match_hk(seuratanno$seurat_clusters, chatti$celltypenum), res_gpt4.5pre_30_ud]]


combined_seurat$celltype_chatti4_5_pur = str_split(combined_seurat$celltype_chatti4_5, "\\:") %>% sapply("[", 2)
combined_seurat$celltype_chatti4_5_pur %>% mytable()


seuratanno[, celltype_chatti4_5_pur := str_split(celltype_chatti4_5, "\\:") %>% sapply("[", 2)]
seuratanno[, .N,celltype_chatti4_5_pur]


ggplotSankey(seuratanno[,.(sample, celltype_chatti4_5)])
ggplotSankey(seuratanno[,.(celltype_chatti4_5, sample )])
ggplotSankey(seuratanno[,.(celltype_chatti4_5_pur, sample )])

DimPlot(combined_seurat, group.by = "celltype_chatti4_5", label = T, repel = T) 
...
# Save
```{r}
fwrite(seuratanno, here("results/05_1_seuratanno_celltypes_superliberalfiltered.txt.gz"), sep = "\t")
saveRDS(combined_seurat, here("results/05_1_seurat_celltypes_superliberalfiltered.rds"))
```
