---
title: "integrate with older and atlas"
format:
  html:
    df-print: paged
    fig-width: 10
...
both10x_pre = merge(x = all10x5, y = karen, add.cell.ids = c("karenstretch", "karenpur"))
both10x_pre
```
```{r}
healthatlas_pre = readRDS("J:/02_projekte/2412_scRNA_lung_organoid_chip/external_scdata/ integrated cell atlas of the human lung core/b351804c-293e-4aeb-9c4c-043db67f4540.rds")
healthatlas_pre


healthatlas_pre$tissue %>% mytable()

plotly::ggplotly( DimPlot(healthatlas_pre[, healthatlas_pre$assay =="10x 3' v3" & healthatlas_pre$tissue =="lung parenchyma"], group.by=c("ann_finest_level")))

( DimPlot(healthatlas_pre[, healthatlas_pre$assay =="10x 3' v3" & healthatlas_pre$tissue =="lung parenchyma"], group.by=c("ann_coarse_for_GWAS_and_modeling","ann_finest_level"), label = TRUE, repel = TRUE) + NoLegend())+ ggplotSankey(healthatlas_pre@meta.data[,c("ann_coarse_for_GWAS_and_modeling"), drop = FALSE])+ ggplotSankey(healthatlas_pre@meta.data[,c( 'ann_finest_level'), drop = FALSE])
table(healthatlas_pre@meta.data[,c("ann_coarse_for_GWAS_and_modeling"), drop = T])

table(healthatlas_pre@meta.data[,c("ann_finest_level"), drop = T])
DimPlot(healthatlas_pre[, healthatlas_pre$assay =="10x 3' v3" & healthatlas_pre$tissue =="lung parenchyma" & healthatlas_pre$ann_coarse_for_GWAS_and_modeling=="Basal"], group.by=c("ann_coarse_for_GWAS_and_modeling","ann_finest_level"), label = TRUE, repel = TRUE)

anno_mark = healthatlas_pre@meta.data %>% as.data.table(keep.rownames = T)

anno_mark[,.N, .(assay)][order(N)]
healthatlas = healthatlas_pre[, healthatlas_pre$assay =="10x 3' v3" & healthatlas_pre$tissue =="lung parenchyma"]
healthatlas
anno_mark2 = anno_mark[assay == "10x 3' v3"& tissue =="lung parenchyma"]
anno_mark2[,.N, .(assay, tissue,lung_condition,fresh_or_frozen,cause_of_death, disease,sequencing_platform,tissue_dissociation_protocol, tissue_sampling_method,development_stage,subject_type)]

anno_mark2[,.N,.(donor_id, sex, self_reported_ethnicity,tissue_type, tissue_level_2, tissue_level_3, sample)]
healthatlas = DietSeurat(healthatlas)
healthatlas[["RNA"]]$data <- NULL
healthatlas[["RNA"]]$scale.data <- NULL
healthatlas$sample %>%  table()
healthatlas$run10x = as.character(healthatlas$sample)
healthatlas$run10x %>%  table()

anno_mark2[, ann_coarse_for_GWAS_and_modeling := as.character(ann_coarse_for_GWAS_and_modeling)]
anno_mark2[, ann_coarse_for_GWAS_and_modeling %>% table() %>% sort()]

anno_mark2[, ann_finest_level := as.character(ann_finest_level)]
anno_mark2[, ann_finest_level %>% table() %>% sort()]


anno_mark2[ann_coarse_for_GWAS_and_modeling !=ann_finest_level,.N, .(ann_coarse_for_GWAS_and_modeling,ann_finest_level)][order(ann_coarse_for_GWAS_and_modeling)] %>% data.frame()

healthatlas$celltype_provis =  healthatlas$ann_coarse_for_GWAS_and_modeling
DimPlot(healthatlas)
```
...
```{r}
both10x = merge(x = both10x_pre, y = healthatlas, add.cell.ids = c("", "atlas"))
both10x
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_rpca"))==FALSE) {
  ...
  fwrite(chatti_rpca, here("results/12_chatti_celltypes_gpt41_rpca"), sep = "\t")
} else chatti_rpca = fread(here("results/12_chatti_celltypes_gpt41_rpca"))
chatti_rpca
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_harmony"))==FALSE) {
  ...
  fwrite(chatti_harmony, here("results/12_chatti_celltypes_gpt41_harmony"), sep = "\t")
} else chatti_harmony = fread(here("results/12_chatti_celltypes_gpt41_harmony"))
chatti_harmony
...
plotly::ggplotly(DimPlot(new_seurat2, group.by = c('run10x', "celltype_provis"), label = T,repel = T,raster=FALSE,  reduction = "umap.harmony"))
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_scvi"))==FALSE) {
  ...
  fwrite(chatti_scvi, here("results/12_chatti_celltypes_gpt41_scvi"), sep = "\t")
} else chatti_scvi = fread(here("results/12_chatti_celltypes_gpt41_scvi"))
chatti_scvi
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_mnn"))==FALSE) {
  ...
  fwrite(chatti_mnn, here("results/12_chatti_celltypes_gpt41_mnn"), sep = "\t")
} else chatti_mnn = fread(here("results/12_chatti_celltypes_gpt41_mnn"))
chatti_mnn
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_jpca"))==FALSE) {
  ...
  fwrite(chatti_jpca, here("results/12_chatti_celltypes_gpt41_jpca"), sep = "\t")
} else chatti_jpca = fread(here("results/12_chatti_celltypes_gpt41_jpca"))
chatti_jpca
...
if(file.exists(here("results/12_chatti_celltypes_gpt41_cca"))==FALSE) {
  ...
  fwrite(chatti_cca, here("results/12_chatti_celltypes_gpt41_cca"), sep = "\t")
} else chatti_cca = fread(here("results/12_chatti_celltypes_gpt41_cca"))
chatti_cca
...
fwrite(anno_new_seurat2, here("results/12_1_anno_all10x_celltypes_qced_integrated_withUnstretched.txt.gz"), sep = "\t")
saveRDS(new_seurat2, here("results/12_1_anno_all10x_celltypes_qced_integrated_withUnstretched.rds"))
...
writexl::write_xlsx(list(cellmarkers_rpca = cellmarkers_rpca), here("results/12_1_cellmarkers_rpca_clust_with_unstretched.xlsx"))
```
