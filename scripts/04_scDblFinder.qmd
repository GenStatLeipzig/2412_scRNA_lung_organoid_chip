...
```{r}
s1 = readRDS(here("../gitOrganoid_hk/results/cellranger1/cellranger/mtx_conversions/S1/S1_filtered_matrix.seurat.rds" ))
s2 = readRDS(here("../gitOrganoid_hk/results/cellranger1/cellranger/mtx_conversions/S2/S2_filtered_matrix.seurat.rds" ))
s3 = readRDS(here("../gitOrganoid_hk/results/cellranger1/cellranger/mtx_conversions/S3/S3_filtered_matrix.seurat.rds" ))
s4 = readRDS(here("../gitOrganoid_hk/results/cellranger1/cellranger/mtx_conversions/S4/S4_filtered_matrix.seurat.rds" ))



# Option 2: Using a list-based approach (more flexible)
seurat_list_pre <- list(S1 = s1, 
                    S2 = s2, 
                    S3 = s3, 
                    S4 = s4)
...
Idents(combined_seurat) = "seurat_cluster"
Idents(combined_seurat) %>% mytable()
avex = AverageExpression(combined_seurat, group.by = "orig.ident")
...
mytable(seurat$scDblFinder.class)


(DimPlot(seurat, group.by = c("seurat_clusters"), label = TRUE) + NoLegend()) + 
  FeaturePlot(seurat, features =  "scDblFinder.score") +
  (VlnPlot(seurat, features = "scDblFinder.score", group.by = "seurat_clusters", pt.size = 0) + scale_y_continuous(breaks = pretty_breaks(10), labels =  label_percent(accuracy = 1)) + geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA)) +
