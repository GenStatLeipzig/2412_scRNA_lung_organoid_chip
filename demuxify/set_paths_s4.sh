#!/bin/bash
# filepath: demuxify/set_paths_s4.sh
# source set_paths_s4.sh before running demuxify/Demuxafy
...
# echo "JSON: $JSON"
# echo "N_DOUB: $N_DOUB"
# source demuxify/set_paths_s4.sh && mkdir -p "$OUTDIR" "$SCDS_OUTDIR" "$SCDBLFINDER_OUTDIR" "$SCRUBLET_OUTDIR" "$DOUBLETFINDER_OUTDIR" "$DOUBLETDETECTION_OUTDIR" "$SOLO_OUTDIR" 

# source demuxify/set_paths_s4.sh && singularity exec --bind /net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/ demuxify/Demuxafy.sif solo -o $SOLO_OUTDIR -j $JSON -d $COUNTS -e $N_DOUB  2>&1 | tee $SOLO_OUTDIR/run.log
...
# source demuxify/set_paths_s4.sh && singularity exec --bind /net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/ demuxify/Demuxafy.sif Scrublet.py -m $COUNTS -o $SCRUBLET_OUTDIR -f $BARCODES 2>&1 | tee $SCRUBLET_OUTDIR/run.log

# source demuxify/set_paths_s4.sh && singularity exec --bind /net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/ demuxify/Demuxafy.sif scDblFinder.R -o $SCDBLFINDER_OUTDIR -t $COUNTS -b $BARCODES_FILTERED 2>&1 | tee $SCDBLFINDER_OUTDIR/run.log

# source demuxify/set_paths_s4.sh && singularity exec --bind /net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/ demuxify/Demuxafy.sif scds.R -o $SCDS_OUTDIR -t $COUNTS -b $BARCODES 2>&1 | tee $SCDS_OUTDIR/run.log
...
COUNTS=/net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/gitOrganoid_hk/results/cellranger1/cellranger/count/S4/outs/filtered_feature_bc_matrix ## Change this based on the path on your system
BARCODES=/net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/gitOrganoid_hk/results/cellranger1/cellranger/S4/cellbender_removebackground/S4_cell_barcodes.csv ## Change this based on the path on your system
...
N_DOUB=$DOUBLETS
OUTDIR=/net/ifs1/san_projekte/projekte/genstat/02_projekte/2412_scRNA_lung_organoid_chip/gitOrganoid_hk/demuxify/S4
